...
*/
    ...
    public static ArrayList<OneEarthquake> fetchEarthquakeData(String url){
        ...
        try{
            jsonToParse = makeHttpRequest(urlToRequest);
        }catch (IOException e){
            Log.e("QUERYUTILS","Problem making the HTTP request.",e);
        }

        ArrayList<OneEarthquake> earthquakes = extractEarthquakes(jsonToParse);
        return earthquakes;
    }
    ...
    private static String makeHttpRequest(URL url) throws IOException{
        ...
        if (url == null) {
            Log.i("QUERYUTILS", "The passed in url is invalid");
            return jsonResponse;
        }
        ...
        try {
            ...
            if (urlConnection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                jsonResponse = convertFromStream(is);
            } else {
                Log.i("QUERYUTILS", "response code from url connection is " + urlConnection.getResponseCode());
            }
        } catch (IOException e) {
            Log.e("QUERYUTILS", "URL CONNECTION ERROR " + e);
        } finally {
    ...
    private static ArrayList<OneEarthquake> extractEarthquakes(String jsonResponse){
        if (jsonResponse == null) {
            Log.e("QUERYUTILS", "JSON RESPONSE IS NULL");
            return null;
        }
        ...
        try {
            ...
            for (int i = 0; i < features.length(); i++) {
                ...
                JSONObject properties = (JSONObject) earthquake.getJSONObject("properties");
                double mag = properties.getDouble("mag");
                String place = properties.getString("place");
                long time = properties.getLong("time");
                String url = properties.getString("url");
                earthquakes.add(new OneEarthquake(mag, place, time, url));
            }
        } catch (JSONException e) {
            Log.e("QUERYUTILS", "JSON PARSING ERROR" + e);
        }
        return earthquakes;
    }
    ...
    private static URL createUrl(String url){
        ...
        try {
            ...
            Log.e("QUERYUTILS", "MalformedURL: " + url + e);
        }
        return urlToRequest;
    }
}
